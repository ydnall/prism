<script lang="ts">
	...
	const cards = $derived([
		...
		{ value: formatSeconds(stats.avgResponseTime), label: 'avg response' },
		{ value: formatSeconds(stats.fastestResponse), label: 'fastest response' },
		{ value: formatSeconds(stats.slowestResponse), label: 'slowest response' },
		{ value: String(stats.instinctAnswers), label: 'instinct picks' },
		{ value: String(stats.timedOutAnswers), label: 'timeouts' }
	]);
...
<div class="w-full max-w-xl text-center">
	<div class="py-6 sm:px-10 sm:py-8">
		<h2 class="text-text-muted mb-4 text-xl font-bold sm:mb-6 sm:text-2xl">Session Complete</h2>

		<!-- Mobile: Swipeable cards -->
		<div class="relative mb-6 sm:hidden">
			<!-- Card container -->
			<div
				class="bg-surface rounded-2xl p-5"
			...
			<!-- Swipe hint -->
			<p class="text-text-muted/50 mt-3 text-xs">Swipe or tap to see more</p>
		</div>

		<!-- Desktop: Grid layout -->
		<div class="mb-6 hidden grid-cols-2 gap-4 sm:grid">
